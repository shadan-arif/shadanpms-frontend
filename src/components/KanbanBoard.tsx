...
const KanbanBoard = () => {
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const destPosts = sourceColumn === destColumn ? sourcePosts : [...destColumn.posts];
    
    const [movedPost] = sourcePosts.splice(source.index, 1);
    destPosts.splice(destination.index, 0, movedPost);
    
    const newColumns = [...columns];
    newColumns[sourceColumnIndex] = { ...sourceColumn, posts: sourcePosts, count: sourcePosts.length };
    
    if (sourceColumn !== destColumn) {
      newColumns[destColumnIndex] = { ...destColumn, posts: destPosts, count: destPosts.length };
    }
  ...
  const handleModalSubmit = (formData: Partial<Post>) => {
    if (editingPost) {
      ...
      const newPost: Post = {
        ...
        description: formData.description || 'Create a marketing banner for the campaign',
        ...
        image: formData.image || '/lovable-uploads/6cc9ce2f-f1e2-40fe-aeaf-c8c9332f7f84.png',
            ...
            <div key={column.id} className="bg-white rounded-lg">
              <div className="flex items-center justify-between p-4 border-b border-gray-200">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-full">{column.count}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <button 
                  ...
                  >
                    {column.posts.length === 0 ? (
                      <div className="text-center py-20">
                        ...
                        <p className="text-gray-500 text-sm mb-4">No content currently. Board is empty</p>
                        <button 
                          onClick={() => handleAddContent(column.id)}
